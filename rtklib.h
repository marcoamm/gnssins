...
#define SYS_IRN     0x40                /* navigation system: IRNS */            // PATCH SEPTENTRIO
...
#define TSYS_IRN    6                   /* time system: IRNSS time */            // PATCH SEPTENTRIO
...
#define NSYSIRN     0
#endif
#define NSYS        (NSYSGPS+NSYSGLO+NSYSGAL+NSYSQZS+NSYSCMP+NSYSIRN) /* number of systems */
...
#define MAXRCVFMT    13
#endif
// PATCH SEPTENTRIO: RENUMEROTATION
...
typedef struct {        /* SSR correction type */
    ...
    float pbias[MAXCODE]; /* phase biases (cycle) */
    double yawAngle;    /* Yaw angle */
    double yawRate;     /* Yaw rate */
    int discontinuity[MAXCODE]; /* discontinuity indicator */
    int n1i[MAXCODE]; /* N1 indicator */
    int wli[MAXCODE]; /* Nw indicator */
...
#define MAXVTEC 15
typedef struct {        /* Intro of Vertical Total Electron Content (VTEC) in ionosphere */
  int nDeg, nOrd;
  double height;
  double sinCoeffs[(MAXVTEC+1)*(MAXVTEC+2)/2];
  double cosCoeffs[(MAXVTEC+1)*(MAXVTEC+2)/2];
} vtec_t;

typedef struct {
  int sys, prn, ambFixed;
  double ion, covion;
} satatmo_t;

#define MAXPRNATMO 40
typedef struct {
  gtime_t time;
  double x,y,z;
  double ztd, covztd;
  int nsat;
  satatmo_t satatmo[MAXPRNATMO];
} staatmo_t;
...
typedef struct {        /* RTCM control struct type */
    ...
    vtec_t vtec;        /* Ionospheric VTEC */
    atmo_t atmo;        /* atmospheric data */
    dgps_t *dgps;       /* output of dgps corrections */
    ssr_t ssr[MAXSAT];  /* output of ssr corrections */
    int disp, mw;       /* additional phase indicators */
...
extern int test_glostr(const unsigned char *buff);                     // PATCH SEPTENTRIO
extern int decode_glostr(const unsigned char *buff, geph_t *geph);     // PATCH SEPTENTRIO
extern int decode_gal_inav(const unsigned char *buff, eph_t *eph);     // PATCH SEPTENTRIO
...
extern int input_sbf   (raw_t *raw, unsigned char data);  // PATCH SEPTENTRIO
extern int input_sbff  (raw_t *raw, FILE *fp);            // PATCH SEPTENTRIO
...
extern int lambda(int n, int m, const double *a, const double *Q, double *F,
                  double *s);
extern void lambdaw(int *r, double *F, double *s, int *n, int *m, const double *a, const double *Q);
...
/*cnesstec for atmospheric message functions----------------------------------*/
