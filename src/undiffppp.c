...
/* FOR relpos, Accomodate for uncombined PPP ************************
...
#define IT(r,opt)   (NP(opt)+NT(opt)/2*(r)) /* tropos (r:0=rov,1:ref)
#define II(s,opt)   (NP(opt)+NT(opt)+(s)-1)   /* ionos (s:satellite no)
...
extern void pppoutsolstat1(rtk_t *rtk, int level, FILE *fp)
{
    ssat_t *ssat;
...
*-----------------------------------------------------------------------------*/
extern void tidedisp1(gtime_t tutc, const double *rr, int opt, const erp_t *erp,
...
/* dual-frequency ***UNCOMBINED measurements -------------------------------------*/
                  ...
                  double phw, double *meas, double *var)
{
    ...
    /* antenna phase center variation correction */
        if (dants) {
          meas[0]-=dants[0];meas[1]-=dants[1];
          meas[2]-=dants[0];meas[3]-=dants[1];}
        if (dantr) {
          meas[0]-=dantr[0];meas[1]-=dantr[1];
          meas[2]-=dantr[0];meas[3]-=dantr[1];
        }
      //  printf("Meas inside IFmEAS:%lf, %lf, %lf, %lf\n",meas[0],meas[1],meas[2],meas[3]);
    return 1;
}
...
static void udionundiff(rtk_t *rtk, double tt)
{
    double el,fact;
    int i,j;
                   ...
                   double *H, double *R, double *azel)
{
    ...
    double didxi=0.0,didxj=0.0,fi,fj,lami,lamj;
    const double *lam=nav->lam[obs->sat-1];
    lami=CLIGHT/lam[0];
    lamj=CLIGHT/lam[1];

    //printf("L1: %lf, L2:%lf, f1: %lf, %lf\n", lam[0],lam[1], (lami*lami)/(lamj*lamj));
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        sat=obs[i].sat;
      //  printf("NP(opt): %d, IC(s,opt): %d, IT(opt): %d, NR(opt): %d, IB(s,opt): %d, NX(opt): %d\n",
      //  NP(opt),IC(0,opt),IT(opt),NR(opt),IB(sat,opt), NX(opt) );
      //  printf("**SAT %d, L1:%lf,L2:%lf\n", sat,nav->lam[sat-1][0],nav->lam[sat-1][1]);
    ...
    //    printf("L1:%lf, L2: %lf, P1: %lf, P2: %lf\n",meas[0], meas[1], meas[2], meas[3]);
        ...
        for (j=0;j<4;j++) { /* for phase and code f1 and f2 */
            ...
            /*INCLUDE IONO  */
              ...
              if (opt->ionoopt==IONOOPT_EST) {
                ...
                if (j==0 || j==2) { //L1
                  didxi=(j==0?-1.0:1.0)*1;
              //    printf("j(0,1,2.3): %d, %lf\n",j,didxi);
                  //v[nv]-=didxi*x[II(sat[i],opt)];???
                  //H[II(sat[i],opt)]= didxi;

                }else { //L2
                  didxi=(j==1?-1.0:1.0)*(lami*lami)/(lamj*lamj);
                //  printf("j(0,1,2.3): %d, %lf\n",j,didxi);
                  //v[nv]-=didxi*x[II(sat[i],opt)];???
                  //H[II(sat[i],opt)]= didxi;
                }
            ...
            if (j==0 || j==1) rtk->ssat[sat-1].resc[0]=v[nv];
            ...
            if (j==0 || j==1) rtk->ssat[sat-1].vsat[0]=1;
            nv++;
        }
    }
...
extern int pppnx1(const prcopt_t *opt)
{
    return NX(opt);
}
...
extern void undiffppp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    /* Verifying how obs are arranged
    for (i=0;i<n;i++) {
        time2str(obs[i].time,str,3);
        satno2id(obs[i].sat,id);
        printf("(%2d) %s %-3s rcv%d %13.3f %13.3f %13.3f %13.3f %d %d %d %d %3.1f %3.1f\n",
              i+1,str,id,obs[i].rcv,obs[i].L[0],obs[i].L[1],obs[i].P[0],
              obs[i].P[1],obs[i].LLI[0],obs[i].LLI[1],obs[i].code[0],
              obs[i].code[1],obs[i].SNR[0]*0.25,obs[i].SNR[1]*0.25);
    }*/

    /* Number of float parameters (rtk->nx) and observation (n)*/
    trace(3,"pppos   : nx=%d n=%d\n",rtk->nx,n);

    /* rs - satellite pos. and vel. {x,y,z,vx,vy,vz} (m,m/s) */
    rs=mat(6,n);
    /* dts - obs[i] sat clock {bias,drift} (s|s/s) */
    dts=mat(2,n);
    /* obs[i] sat position and clock error variance (m^2)*/
    var=mat(1,n);
    /* Satellite azimuth and elevation angle vector */
    azel=zeros(2,n);
  ...
  printf("NP(opt): %d, IC(s,opt): %d, IT(opt): %d, NR(opt): %d, IB(s,opt): %d, NX(opt): %d\n",
  NP(opt),IC(0,opt),IT(opt),NR(opt),IB(0,opt), NX(opt) );


    printf("%d H %d \n",rtk->nx,nv);
    for (i = 0; i < nv; i++) {
      for (j = 0; j < rtk->nx; j++) {
        printf("%lf\t",H[i*(rtk->nx)+j]);
      }
      printf("\n");
    }
    printf("***********************************************************\n\n\n");
